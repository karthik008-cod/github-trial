package javaInput;
import java.util.*;
public class LinearEquationsBasic8 {
    ...
    public static void main(String[] args) {
        ...
        for(int i=1;i<=n;i++){
            ...
            int coefficient1 = random.nextInt(1,11);
            int coefficient2 = random.nextInt(1,11);
            int coefficient3 = random.nextInt(1,11);
            int coefficient4 = random.nextInt(1,11);
            int power = random.nextInt(1,4);
            if (type == 1){
                System.out.println(i+". ("+coefficient2+symbol()+Power()+symbol()+" + "+coefficient3+symbol()+Power()+symbol()+")("+coefficient1+symbol()+Power()+symbol()+" + "+coefficient4+symbol()+Power()+symbol()+")");
            }else if (type == 2){
                System.out.println(i+". ("+coefficient2+symbol()+Power()+symbol()+" - "+coefficient3+symbol()+Power()+symbol()+")("+coefficient1+symbol()+Power()+symbol()+" - "+coefficient4+symbol()+Power()+symbol()+")");
            }else if (type == 3){
                System.out.println(i+". ("+coefficient2+symbol()+Power()+symbol()+" + "+coefficient3+symbol()+Power()+symbol()+")("+coefficient1+symbol()+Power()+symbol()+" - "+coefficient4+symbol()+Power()+symbol()+")");
